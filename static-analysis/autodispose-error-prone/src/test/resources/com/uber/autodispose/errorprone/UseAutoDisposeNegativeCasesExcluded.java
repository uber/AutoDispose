...
public class UseAutoDisposeNegativeCasesExcluded implements LifecycleScopeProvider<TestLifecycle> {
  ...
  public void parallelFlowable_subscribeWithAutoDispose() {
    ...
    Flowable.just(1, 2).parallel(2).subscribe(subscribers);
  }

  public void observable_subscribeVoidSubscribe() {
    Observable.just(1).subscribe(new TestObserver<>());
  }

  public void single_subscribeVoidSubscribe() {
    Single.just(1).subscribe(new TestObserver<>());
  }

  public void completable_subscribeVoidSubscribe() {
    Completable.complete().subscribe(new TestObserver<>());
  }

  public void maybe_subscribeVoidSubscribe() {
    Maybe.just(1).subscribe(new TestObserver<>());
  }

  public void flowable_subscribeVoidSubscribe() {
    Flowable.just(1).subscribe(new TestSubscriber<>());
  }

  public void observable_subscribeWith() {
    Observable.just(1).subscribeWith(new TestObserver<>());
  }

  public void single_subscribeWith() {
    Single.just(1).subscribeWith(new TestObserver<>());
  }

  public void completable_subscribeWith() {
    Completable.complete().subscribeWith(new TestObserver<>());
  }

  public void maybe_subscribeWith() {
    Maybe.just(1).subscribeWith(new TestObserver<>());
  }

  public void flowable_subscribeWith() {
    Flowable.just(1).subscribeWith(new TestSubscriber<>());
  }
}
