...
class TestKotlinActivity : AppCompatActivity(), ScopeProvider {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    Observable.interval(1, TimeUnit.SECONDS).autoDispose(this).subscribe()
    ...
    Maybe.just(1).autoDispose(this).subscribe()

    Maybe.just(1).autoDispose(this, Lifecycle.Event.ON_DESTROY).subscribe()

    Flowable.just(1).autoDispose(this).subscribe()

    Flowable.just(1).autoDispose(this, Lifecycle.Event.ON_DESTROY).subscribe()

    Single.just(1).autoDispose(this).subscribe()

    Single.just(1).autoDispose(this, Lifecycle.Event.ON_DESTROY).subscribe()

    Completable.never().autoDispose(this).subscribe()

    Completable.never().autoDispose(this, Lifecycle.Event.ON_DESTROY).subscribe()

    val rootView = findViewById<View>(android.R.id.content)

    // Taking scope of a View
    Observable.interval(1, TimeUnit.DAYS).autoDispose(rootView).subscribe()
